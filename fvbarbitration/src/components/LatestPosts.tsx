  ...
  return (
    <div>
        ...
        {posts.map((post) => (
          ...
          >
            {post.featured_image_url && (
              <img src={post.featured_image_url} alt={post.title} width={400} height={225} className="h-40 w-full object-cover" />
            )}
            <div className="p-5">
              ...
              {post.meta_description && (
                <p className="mt-2 text-sm text-gray-600 line-clamp-2">{post.meta_description}</p>
              )}
            </div>
          </article>
        ))}
      ...
      <div className="mt-8 text-center">
        <Link href={`/${locale}/blog/`} className="text-accent font-medium hover:underline">{t.viewAll} &rarr;</Link>
      </div>
    </div>
  );
}
