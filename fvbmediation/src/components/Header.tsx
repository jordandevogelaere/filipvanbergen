...
export default function Header({ locale }: { locale: Locale }) {
  ...
  useEffect(() => {
    fetch(`${BLOG_API_URL}/settings?site=fvbmediation`)
      .then((r) => r.json())
      .then((data) => {
  ...
  return (
    <header className="bg-tan-900 fixed top-0 z-50 w-full">
      ...
      {mobileOpen && (
        <nav aria-label={locale === "en" ? "Main navigation" : locale === "fr" ? "Navigation principale" : "Hoofdnavigatie"} className="bg-tan-900 border-tan-700 border-t px-6 pb-4 md:hidden">
          {navLinks.map((link) => (
            <Link
              key={link.href}
              href={link.href}
          ...
          <div className="border-tan-700 mt-2 border-t pt-3">
            <LanguageSwitcher locale={locale} />
          </div>
        </nav>
      )}
